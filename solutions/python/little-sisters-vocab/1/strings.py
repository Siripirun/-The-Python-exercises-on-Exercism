...
def remove_suffix_ness(word):
    ...
    root_word = word[:-4] # Remove 'ness'
    if root_word.endswith('i'): # Adjust spelling if root word ends with 'i'
        root_word = root_word[:-1] +'y' # Adjust 'i' to 'y'
    return root_word
...
def adjective_to_verb(sentence, index):
    ...
    adjective = adjective.rstrip('.,;!?')  # Remove any trailing punctuation
    return adjective + 'en'  # Convert adjective to verb by adding 'en'
